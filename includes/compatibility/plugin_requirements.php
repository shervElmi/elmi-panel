...
class Plugin_Requirements {
		...
		if ( version_compare( PHP_VERSION, $this->get_php_version(), '<' ) ) {
			$message = esc_html(
				sprintf(
					...
					__( 'Elmi Panel requires PHP %s or higher.', 'elmi-panel' ),
		...
		if ( version_compare( get_bloginfo( 'version' ), $this->get_wp_version(), '<' ) ) {
			$message = esc_html(
				sprintf(
					...
					__( 'Elmi Panel requires WordPress %s or higher.', 'elmi-panel' ),
					$this->get_wp_version()
				)
			);
				...
				if ( ! is_readable( $required_file ) ) {
					$message =
						sprintf(
							...
							__( 'You appear to be running an incomplete version of the plugin. Please run %s to finish installation.', 'elmi-panel' ),
	 ...
	 * @return self
	 */
	public function set_required_theme( string $required_theme ) {
